...
if (selectedSession === 'yes') {
    let conversationHistoryStr = localStorage.getItem(`conversationHistory_${selectedModel}`);
    try {
        conversationHistory = conversationHistoryStr ? JSON.parse(conversationHistoryStr) : [];
    } catch (error) {
